...
{
	$rpName = "Blob Share Local";
	$groupName = "Default Local Rule Group for $rpName";
	$rpReturnUrl = $relyingPartyRealm + "Account/LogOn";
}
else
{
	$rpName = "Blob Share Azure";
	$groupName = "Default Azure Rule Group for $rpName";
	$rpReturnUrl = $relyingPartyRealm + "Account/LogOn";
}
...
foreach ($rp in $rps)
{
    ...
    {
        Write-Host -fore red "Error: There is already a relying party with the realm: '" $relyingPartyRealm "'. Choose a different realm for the Blob Share application and try again."
        exit 1;        
    }
}
